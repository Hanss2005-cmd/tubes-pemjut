      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-1/2 p-12 lg:p-16 flex flex-col justify-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button 
              onClick={() => handleWAContact(`${WA_LINK}?text=${encodeURIComponent(`Halo Mulya Cartridge, saya tertarik dengan produk:
*${product.name}*
      ...
      <div className="max-w-7xl mx-auto">
        <div id="admin_stats" className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium text-lg mt-2">Pusat kontrol & intelijen bisnis Mulya Cartridge</p>
          </div>
          <div className="flex gap-4">
            ...
            <button onClick={() => { setShowAdminPanel(false); window.scrollTo(0,0); }} className={`px-8 py-4 rounded-2xl font-bold border transition-all ${isDarkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-lg'}`}>Tutup Panel</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {[
            ...
            { label: "Knowledge Base", value: expertRules.length, icon: ShieldCheck, color: "purple" },
            { label: "Database Link", value: "Active", icon: Database, color: "emerald", isStatus: true }
          ].map((stat, i) => (
            <motion.div 
              key={i}
            ...
            >
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${
                ...
                stat.color === 'purple' ? 'bg-purple-500/10 text-purple-500' : 'bg-emerald-500/10 text-emerald-500'
              }`}>
          ...
          {expertStep === 5 && (
            <div key="step5" className="space-y-6 animate-in fade-in zoom-in duration-300">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-dashed border-slate-200 dark:border-slate-700">
                <button 
                  onClick={() => handleWAContact(`${WA_LINK}?text=Halo Mulya Cartridge, saya melakukan diagnosa mandiri untuk ${expertCategory} ${expertType}. Hasil diagnosa: ${diagnosisResults.map(r => r.diagnosis).join(", ")}. Saya ingin konsultasi perbaikan.`)}
...
export default function App() {
  ...
  const scrollTo = (id: string) => {
    ...
    if (['hero', 'about', 'services', 'products', 'location'].includes(id) && showAdminPanel) {
      setShowAdminPanel(false);
    }
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center gap-8">
              {showAdminPanel && isAdmin ? (
                <>
                  <button onClick={() => scrollTo('admin_stats')} className={`text-sm font-bold transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'}`}>Dashboard</button>
                  <button onClick={() => scrollTo('admin_products')} className={`text-sm font-medium transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'}`}>Produk</button>
                  <button onClick={() => scrollTo('admin_services')} className={`text-sm font-medium transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'}`}>Layanan</button>
                  <button onClick={() => scrollTo('admin_expert')} className={`text-sm font-medium transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'}`}>Sistem Pakar</button>
                  <button onClick={() => { setShowAdminPanel(false); window.scrollTo(0,0); }} className="text-sm font-black text-rose-500 uppercase tracking-widest px-4 py-1.5 rounded-lg border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500 hover:text-white transition-all">Lihat Web</button>
                </>
              ) : (
                <>
                  <button onClick={() => scrollTo('hero')} className={`text-sm font-medium transition-colors uppercase tracking-wider ${isDarkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'}`}>Home</button>
            ...
            >
              {showAdminPanel && isAdmin ? (
                <>
                  ...
                  <button onClick={() => { setShowAdminPanel(false); setIsMenuOpen(false); window.scrollTo(0,0); }} className="text-left py-2 font-bold text-rose-500">LIHAT WEB UTAMA</button>
                </>
              ) : (
                <>
                  <button onClick={() => { scrollTo('hero'); setIsMenuOpen(false); }} className="text-left py-2 font-medium">Home</button>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16 text-center md:text-left">
                <div>
                  ...
                  <p className="text-slate-500 font-medium text-lg mt-2">Menampilkan seluruh inventori produk Mulya Cartridge</p>
                </div>
                    ...
                    {[
                      {
                        ...
                        type: "image",
                        media: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=2000&auto=format&fit=crop"
                      },
                      {
                        year: "Pertengahan 2004",
                        title: "Lahirnya Mulya Cartridge",
                        description: "Resmi mendirikan CV. Mulya Cartridge di Jaya Plaza Computer Mall Lt. 2 Bandung. Dimulai sebagai tim kecil bersama istri, kami fokus pada kualitas produk dan pelayanan dari hati ke hati.",
                        type: "image",
                        media: "/fotomulya.png"
                      },
                      {
                        ...
                        type: "video",
                        media: "/video_workshop.mp4",
                        placeholder: "https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?q=80&w=2000&auto=format&fit=crop"
                      },
                      {
                        year: "Sekarang",
                        ...
                        type: "image",
                        media: "https://images.unsplash.com/photo-1531297484001-80022131f5a1?q=80&w=2000&auto=format&fit=crop"
                      }
                    ].map((item, index) => (
